...
if __name__ == '__main__':
    ...
    csv_header = "Hostname,Port,Types,Speed,Description"
    ...
    with open(host_file_in,"r") as host_file :
        ...
        for host in host_file:
            try:
                ...
                ##get mac table
                print(lineno(), "get int description")
                command_pass = 'sh int desc | exclude "Interface|Port|------------------"'
                output_intd= net_connect.send_command(command_pass)
                ...
                show_int_desc_p1 = []
                show_int_desc_p1 = output_intd.split("\n")
                #
                for elem in show_int_desc_p1:
                    if elem != "":
                        if re.search("^Po|^mgmt0|^Vlan|^Lo", elem):
                            line = ""
                            line = hostname + "," + re.sub(r"[\s]+",",,,",elem,1)
                            #print(lineno(),line + "\n")
                            show_int_desc.append(line)
                        else:
                            line = ""
                            line = hostname + "," + re.sub(r"[\s]+",",",elem,3)
                            show_int_desc.append(line)
                #
                print (lineno() , "No of enties:", len(show_int_desc))
